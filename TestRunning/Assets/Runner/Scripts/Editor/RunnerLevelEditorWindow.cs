...
namespace HyperCasual.Runner
{
    ...
    public class RunnerLevelEditorWindow : EditorWindow
    {
        ...
        const string k_AutoSavePlayerDisabledByDefaultKey = "AutoSavePlayerDisabledByDefault";
        ...
        public void LoadAutoSaveSettings()
        {
            ...
            {
                // Default auto-save values and save them to Editor Prefs
                // the first time the user opens the window

                m_AutoSaveLevel = true;
                m_AutoSavePlayer = false;
                ...
                EditorPrefs.SetBool(k_AutoSavePlayerDisabledByDefaultKey, true);
                return;
            }
            ...
            if (!EditorPrefs.GetBool(k_AutoSavePlayerDisabledByDefaultKey))
            {
                m_AutoSavePlayer = false;
                EditorPrefs.SetBool(k_AutoSavePlayerKey, m_AutoSavePlayer);
                EditorPrefs.SetBool(k_AutoSavePlayerDisabledByDefaultKey, true);
        ...
        void SaveLevel(LevelDefinition levelDefinition)
        {
            if (m_AutoSaveLevel)
            {
                ...
                List<LevelDefinition.SpawnableObject> savedSpawnables = new List<LevelDefinition.SpawnableObject>();
                for (int i = 0; i < spawnables.Length; i++)
                {
                    try
                    {
                        Spawnable spawnable = spawnables[i];
                        if (!ShouldSaveSpawnable(spawnable))
                        {
                            continue;
                        }
                        ...
                        {
                            Debug.LogWarning($"Skipping '{spawnable.name}' because it is not connected to a prefab asset. Only prefab-backed spawnables can be saved and reloaded.", spawnable);
                            continue;
                        }

                        savedSpawnables.Add(new LevelDefinition.SpawnableObject()
                        {
                            SpawnablePrefab = spawnablePrefab,
                            Position = spawnable.SavedPosition,
                            EulerAngles = spawnable.transform.eulerAngles,
                            Scale = spawnable.transform.lossyScale
                        });
                    }
                ...
                levelDefinition.Spawnables = savedSpawnables.ToArray();
            ...
            if (m_AutoSavePlayer)
            {
                ...
                if (players.Length == 1)
                {
                    TryApplyPrefabInstance(players[0].gameObject, nameof(PlayerController), "Save Player on Play");
                }
                else
                {
                    if (players.Length == 0)
                    ...
                    {
                        Debug.LogWarning("More than one instance of PlayerController found in the scene. No changes saved!");
                    }
                }
            }
            ...
            if (m_AutoSaveCamera)
            {
                ...
                {
                    TryApplyPrefabInstance(cameraManagers[0].gameObject, nameof(CameraManager), "Save Camera on Play");
                }
                else
                {
        ...
        bool ShouldSaveSpawnable(Spawnable spawnable)
        {
            if (spawnable == null)
            {
                return false;
            ...
            return m_LoadedLevelGO != null && spawnable.transform.IsChildOf(m_LoadedLevelGO.transform);
        }

        static void TryApplyPrefabInstance(GameObject target, string componentName, string autoSaveSettingName)
        {
            GameObject prefabInstanceRoot = PrefabUtility.GetNearestPrefabInstanceRoot(target);
            if (prefabInstanceRoot == null || !PrefabUtility.IsPartOfNonAssetPrefabInstance(prefabInstanceRoot))
            {
                Debug.LogWarning($"{componentName} on '{target.name}' is not part of a prefab instance, so its scene changes cannot be saved back to a prefab. Disable '{autoSaveSettingName}' or replace the scene object with a prefab instance.", target);
                return;
            }
